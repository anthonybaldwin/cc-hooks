#!/usr/bin/env bash
# CC Hooks — session-color uninstall
#
# Removes the session-color hooks from ~/.claude/settings.json (leaving other
# hooks like notifications intact) and resets the terminal background. The hook
# script itself is left in ~/.claude/hooks/ — delete it manually if desired.
